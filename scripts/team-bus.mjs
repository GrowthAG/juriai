...
async function start() {
  ...
  console.log(`Agent Bus autenticado iniciado em ${origin} (PID ${status.pid}).`);
    ...
    if (pendingInstance) {
      try {
        await proveOwnership(pendingInstance);
        await fetch(`${origin}/admin/shutdown`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${pendingInstance.controlToken}`,
            "X-JuriAI-Instance-Id": pendingInstance.instanceId,
          },
          signal: AbortSignal.timeout(1500),
        });
      } catch {
        // Never target a process that failed ownership proof.
      }
    }
    throw error;
  } finally {
