...
function codexPolicyArgs(policy) {
  if (policy === "read-only") return ["-s", "read-only", "-a", "never"];
  return ["-s", "workspace-write", "-a", "on-request"];
}

function claudePolicyArgs(policy) {
  return ["--permission-mode", policy === "read-only" ? "plan" : "acceptEdits"];
}

function geminiPolicyArgs(policy) {
  return ["--approval-mode", policy === "read-only" ? "plan" : "auto_edit"];
}
...
const policySummary = {
  codex: roleDefaults.writePolicy === "read-only" ? "sandbox=read-only approval=never" : "sandbox=workspace-write approval=on-request",
  claude: roleDefaults.writePolicy === "read-only" ? "permission-mode=plan" : "permission-mode=acceptEdits",
  gemini: roleDefaults.writePolicy === "read-only" ? "approval-mode=plan" : "approval-mode=auto_edit",
}[provider];
