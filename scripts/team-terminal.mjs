...
const roleDefaults = {
  provider: role?.defaultProvider ?? "codex",
  model: role?.defaultModel,
  reviewRole: role?.reviewRole ?? "none",
  approvalScope: role?.approvalScope ?? [],
  writePolicy: role?.writePolicy ?? "read-only",
};
const canWriteCode = roleDefaults.writePolicy === "code-write";
const provider =
  providerAliases[option("--provider")] ?? option("--provider") ?? roleDefaults.provider;
const model = option("--model") ?? roleDefaults.model;
const providers = {
...
function codexPolicyArgs(policy) {
  if (policy === "code-write") return ["-s", "workspace-write", "-a", "on-request"];
  return ["-s", "read-only", "-a", "never"];
}

function claudePolicyArgs(policy) {
  return ["--permission-mode", policy === "code-write" ? "acceptEdits" : "plan"];
}

function geminiPolicyArgs(policy) {
  return ["--approval-mode", policy === "code-write" ? "auto_edit" : "plan"];
}
...
const prompt = [
  ...
  `Leia .agents/TEAM.md, .agents/PRODUCT_STATE.md, .agents/APPROVALS.md e ${role.roleFile}.`,
  ...
  `Sua regra de aprovacao e ${roleDefaults.reviewRole}; escopo: ${
    roleDefaults.approvalScope.length ? roleDefaults.approvalScope.join(", ") : "nenhum"
  }.`,
  `Sua politica de escrita e ${roleDefaults.writePolicy}.`,
  role.key === "coordinator"
    ? "Voce faz o gate interno final e devolve approved, changes_requested ou rejected."
    : "Voce nao e o aprovador final; encaminhe revisao ao terminal revisor adequado.",
].join(" ");
...
function launchConfiguration() {
  if (provider === "codex") {
    const launchArgs = [
      "-C",
      root,
      ...codexPolicyArgs(roleDefaults.writePolicy),
      "-c",
  ...
  if (provider === "claude") {
    const launchArgs = [
      ...
      "--name",
      role.agentId,
      ...claudePolicyArgs(roleDefaults.writePolicy),
    ];
    if (model) launchArgs.push("--model", model);
    launchArgs.push(prompt);
  ...
  if (model) launchArgs.push("--model", model);
  return { binary: path.join(root, "scripts", "gemini-vertex"), args: launchArgs };
}

const launch = launchConfiguration();
const policySummary = {
  codex: canWriteCode ? "sandbox=workspace-write approval=on-request" : "sandbox=read-only approval=never",
  claude: canWriteCode ? "permission-mode=acceptEdits" : "permission-mode=plan",
  gemini: canWriteCode ? "approval-mode=auto_edit" : "approval-mode=plan",
}[provider];
...
if (dryRun) {
  ...
  console.log(`Gate: ${roleDefaults.reviewRole}`);
  console.log(`Write policy: ${roleDefaults.writePolicy}`);
  console.log(`Policy real: ${policySummary}`);
...
const child = spawn(
  ...
  launch.args,
  {
    ...
    env: {
      ...
      JURIAI_AGENT_REVIEW_ROLE: roleDefaults.reviewRole,
      JURIAI_AGENT_APPROVAL_SCOPE: roleDefaults.approvalScope.join(","),
      JURIAI_AGENT_WRITE_POLICY: roleDefaults.writePolicy,
