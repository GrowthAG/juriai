...
test("tokens por agente e RBAC impedem impersonacao", async (t) => {
  ...
  const cybersecurityToken = "s".repeat(64);
  const credentialsPath = path.join(directory, "credentials.json");
  const manifestPath = path.join(directory, "team.json");
  fs.writeFileSync(
    credentialsPath,
    JSON.stringify({
      schemaVersion: 1,
      ...
      agents: {
        ...
        "juriai-cybersecurity": { role: "cybersecurity", token: cybersecurityToken },
      },
    }),
    { mode: 0o600 },
  );
  fs.writeFileSync(
    ...
    JSON.stringify({
      schemaVersion: 1,
      agentBus: { projectId: "security-test" },
      roles: [
        { key: "coordinator", agentId: "juriai-coordinator" },
        { key: "po", agentId: "juriai-po" },
        { key: "cybersecurity", agentId: "juriai-cybersecurity" },
      ],
    }),
    { mode: 0o600 },
  );
  const child = spawn(process.execPath, ["src/server.mjs"], {
    ...
    env: {
      ...
      JURIAI_AGENT_BUS_MANIFEST: manifestPath,
  ...
  const cybersecurity = await connectClient(
    `${origin}/mcp`,
    cybersecurityToken,
    "cybersecurity",
  );
  clients.push(coordinator, po, cybersecurity);
  ...
  for (const [client, agent] of [
    ...
    [cybersecurity, { agentId: "juriai-cybersecurity", role: "cybersecurity" }],
  ]) {
  ...
  const pending = parseResult(
    await call(coordinator, "create_task", {
      title: "fila fechada",
      description: "nao pode ser assumida sem atribuicao",
      createdBy: "juriai-coordinator",
      writeScope: [],
      context: {},
    }),
  );
  const forbiddenPendingClaim = await call(po, "claim_task", {
    taskId: pending.task.taskId,
    agentId: "juriai-po",
    allowConflict: false,
  });
  assert.equal(forbiddenPendingClaim.isError, true);
  ...
  const coordinatorCannotComplete = await call(coordinator, "complete_task", {
    taskId: created.task.taskId,
    agentId: "juriai-coordinator",
    result: { forged: true },
  });
  assert.equal(coordinatorCannotComplete.isError, true);

  const unrelatedRecipient = await call(po, "send_message", {
    fromAgent: "juriai-po",
    toAgent: "juriai-cybersecurity",
    taskId: created.task.taskId,
    kind: "message",
    body: "destinatario nao participa da tarefa",
    metadata: {},
  });
  assert.equal(unrelatedRecipient.isError, true);
  ...
  const wrongShutdown = await fetch(`${origin}/admin/shutdown`, {
    method: "POST",
    headers: {
      Authorization: "Bearer wrong-control-token",
      "X-JuriAI-Instance-Id": "security-instance",
    },
  });
  assert.equal(wrongShutdown.status, 401);
});
