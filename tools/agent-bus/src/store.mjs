...
export class AgentBusStore {
  ...
  event(eventType, actorAgent, taskId, payload = {}) {
    this.db
      .prepare(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `)
      .run(eventType, actorAgent ?? null, taskId ?? null, stringify(payload, {}), now());
  }
  ...
  getAgent(agentId) {
    return mapAgent(this.db.prepare("SELECT * FROM agents WHERE id = ?").get(agentId));
  }
  ...
  listAgents({ workspaceId, includeOffline = true } = {}) {
    ...
    if (!includeOffline) conditions.push("status != 'offline'");
  ...
  handoffTask({ taskId, fromAgent, toAgent, note, allowConflict = false }) {
    ...
    this.event("task.handed_off", fromAgent, taskId, { toAgent, note });
    return { task, message };
  }
  ...
  updateTaskStatus({ taskId, agentId, status, result, error }) {
    ...
    this.event(`task.${status}`, agentId, taskId, { result, error });
    return this.getTask(taskId);
  }
...
export const internals = { normalizeScope, scopesOverlap };
