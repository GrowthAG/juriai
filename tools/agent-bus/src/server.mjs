...
const credentials = loadAgentCredentials(credentialsPath, projectId);
...
function createServer(principal) {
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      if (input.taskId) requireTaskVisibility(input.taskId, "send_message");
...
app.use("/mcp", (req, res, next) => {
  ...
  store.touchPresence(principal.agentId);
  return next();
});
