...
testes com `NODE_ENV=test` e `JURIAI_AGENT_BUS_ALLOW_INSECURE_LOCAL=1`.

O launcher comprova a instancia gerenciada por challenge HMAC antes de liberar
o token do agente. O controller usa lock atomico para impedir dois starts e
recusa sucesso para instancia compativel que nao consiga provar ownership.

Este modelo e um guardrail de coordenacao entre processos confiaveis do mesmo
usuario. Arquivo `0600` nao isola processos executados pelo mesmo UID. Para um
modelo adversarial, execute cada agente sob UID/container/sandbox distinto e
monte apenas a credencial daquele agente.
...
1. O responsavel conclui a implementacao e envia `handoff_task` para o terminal
   revisor do dominio.
2. O revisor responde com `review` ou `decision` por `send_message`.
3. Se houver ajuste, o revisor devolve `changes_requested` com evidencia
   objetiva.
4. O coordenador consolida a revisao interna antes da decisao humana.
...
Este MVP é para agentes locais confiaveis no mesmo computador. Quando agentes
rodarem em outras maquinas, acrescente TLS mutuo, identidade vinculada ao
cliente, autorizacao central e um broker como NATS ou Redis. Se virarem servicos
autonomos descobertos pela rede, A2A pode ser adicionado na borda; o MCP continua
util como interface de ferramentas dentro de cada agente.
