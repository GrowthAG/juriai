...
npm --prefix tools/agent-bus start
```
...
```bash
JURIAI_MOSAIC_WAKE=1 npm --prefix tools/agent-bus start
```
...
Se ativar `JURIAI_AGENT_BUS_TOKEN`, todos os clientes precisam enviar `Authorization: Bearer <token>`. O exemplo do Codex já mostra `bearer_token_env_var`. Para Claude e Gemini, adicione o header conforme a versão do CLI instalada e mantenha o segredo em variável de ambiente, não no Git.
...
O servidor HTTP é um processo separado e não consegue descobrir qual terminal originou uma chamada MCP. Por isso cada agente envia explicitamente seu `agentId` e seu `surfaceId` ao se registrar.
...
- Tarefas: `create_task`, `list_tasks`, `get_task`, `assign_task`, `claim_task`, `handoff_task`, `report_blocker`, `complete_task`, `fail_task`.
...
Os testes cobrem provedores diferentes, presença, inbox, claim, conclusão, handoff e conflito de escopo.
